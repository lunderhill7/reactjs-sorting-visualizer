const algorithmInfos = {
	...
	heap_sort: {
		...
		time_complexity: {
			best: ['O(n log(n))', 'orange-700'],
			average: ['O(n log(n))', 'orange-700'],
			worst: ['O(n log(n))', 'red-800']
		},
		space_complexity: ['O(1)', 'lime-700'],
		description: `
			Heap Sort is a comparison-based sorting algorithm that uses a binary heap data structure (usually a max-heap) to sort elements. It repeatedly removes the largest element from the heap and places it at the end of the array, shrinking the heap and restoring the heap property after each step.
		`
	},
	...
	shell_sort: {
		...
		time_complexity: {
			best: ['O(n log(n))', 'orange-700'],
			average: ['O(n^1.5)', 'orange-700'],
			worst: ['O(n^2)', 'red-800']
		},
		space_complexity: ['O(1)', 'lime-700'],
		description: `
			Shell Sort is an optimized version of Insertion Sort that improves efficiency by allowing the exchange of far-apart elements. It starts by sorting elements that are far apart and gradually reduces the gap between compared elements, leading to fewer swaps and better performance on larger datasets..
		`
	},
